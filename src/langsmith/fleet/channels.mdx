...
The default Slack bot activates your agent when messages are posted in a connected Slack channel. It triggers on every message in the channel and cannot receive DMs. The default bot does not have access to Slack tools, so the agent cannot respond in Slack.

<Tip>
For bidirectional Slack communication, tag-only triggering, or DM support, [create a custom Slack bot](/langsmith/fleet/slack-app) instead. See [Default bot vs. custom bot](/langsmith/fleet/slack-app#default-vs-custom-bot) for a comparison.
</Tip>
