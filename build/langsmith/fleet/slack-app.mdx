...
LangSmith Fleet offers two ways to connect an agent to Slack: the **default Slack bot** and a **custom Slack bot**.

## Default vs. custom bot

| | Default Slack bot | Custom Slack bot |
|---|---|---|
| **Setup** | No configuration required | Requires creating a Slack app |
| **Trigger** | Triggers on every message in a connected channel | Configurable — can trigger on all messages or only when tagged |
| **DMs** | Not supported — channel-only | Supported |
| **Slack tools** | Not available | Available — send messages, reply in threads, read history |
| **Best for** | Dedicated channels where the agent only needs to be triggered | Shared channels where the agent should respond in Slack |
...
## Use the default Slack bot

The default LangSmith Slack bot triggers your agent on every message posted in a connected channel. It cannot receive DMs and does not have access to Slack tools, so it cannot respond in Slack.

Use the default bot when you need a simple trigger to kick off an agent run from a Slack channel. The agent processes the message but responds in the Fleet UI, not in Slack.

1. In the agent editor, scroll to the **Channels** section and click **LangSmith Bot**.
...
1. Send a message in the channel to start a run.

<Tip>
For full bidirectional Slack communication, [create a custom Slack bot](#create-a-custom-slack-bot) instead.
</Tip>
...
<Tip>
If you don't need tag-only triggering or DMs, you can use the [default Slack bot](#use-the-default-slack-bot) instead.
</Tip>
...
Slack tools require a custom Slack bot. They let your agent send messages, reply in threads, and read channel history. Slack tools work regardless of how the agent was triggered, whether through Slack, the Fleet UI, a schedule, or a webhook.
