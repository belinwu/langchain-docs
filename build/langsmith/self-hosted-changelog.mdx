...
- Fixed path prefix for ABAC permission checks to improve self-hosted functionality.
- Fixed handling of DotDict in decrypt_passthrough_headers within Agent Builder.
- Fixed fleet logo to use currentColor for better dark mode support.
- Added network allow denylist for enhanced network security.
- Added access control UI for sandbox proxy to improve UI management.
- Improved CopyObject handling for GCS workload identity and added a copy healthcheck.
- Reduced concurrency and added timeout for scheduled insights jobs to optimize performance.
- Reduced number of preload chunks during initial load to enhance performance.
- Added OpenAI chat completion support.
- Cached /info and /auth/v1/user responses in localStorage for improved performance.
- Fixed backend to recreate feedbacks index to address data issues.
- Added JWT generation for auth-proxy in playground service for enhanced security.
- Enabled skill editing gating in Agent Builder by repo ownership for improved access control.
- Added evaluator JWT generation for LLM-as-judge evaluations.
- Always displayed creator name on agent workspace card for better identification.
- Supported RDS DB instance for k8s platform, enhancing deployment options.
- Fixed JWT signature verification for service key decode in agent-builder-tool-server.
- Supported Google IAP session refresh in self-hosted instances for improved security integration.
- Fixed trace count dashboard issues when grouping by metadata key.
- Replaced MUI checkboxes in settings for UI consistency.
- Added new comparison view details metrics table for better data analysis.
- Added support for specifying environment variables from secret reference in host-backend.
- Added LangChain vendor extractor in the messages feature for better integration support.
- Bumped langgraph-api to 0.7.83 to resolve self-hosted 403 errors.
