...
agent to a [sandbox](/oss/python/javascript/deepagents/sandboxes) so it can read,
...
from agent import sandbox  # import the shared sandbox instance

app = FastAPI()

@app.get("/api/sandbox/tree")
async def list_tree(path: str = Query("/app")):
...
async def read_file(path: str = Query(...)):
    results = await sandbox.download_files([path])
...
See [Sandboxes](/oss/python/javascript/deepagents/sandboxes) for details on per-thread
...
- **Sync files on every tool call**, not just when the run finishes. This
